...
From `GenBankViz/web`, run:

```bash
npm run preview -- --host 127.0.0.1
```

Open <http://127.0.0.1:4173/GenBankViz/>. The browser can select `.gb`, `.gbk`, `.gbff`, `.genbank`, and their `.gz` forms. Press <kbd>Ctrl</kbd>+<kbd>C</kbd> to stop the server.

No graphical launcher is installed. A user-created launcher must set its working directory to `GenBankViz/web`, run the command above, and keep the terminal process alive. Do not configure `--host 0.0.0.0` by default; that exposes the server to other machines that can reach the computer.
